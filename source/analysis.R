...
xm <-
  zoo(data$value, order.by = data$Date) # converti les valeurs de data en série temporelle de type "zoo" en les indiçant par la colonne Date
...
# On inclue le paramètre 20 pour voir l'autocorrélation des résidus sur les 20 premiers lags. 
# Les deux premiers ne sont pas calculés car il y a 2 degrés de liberté en moins à 
# cause du nombre de coefficients de la spécification du test ADF.
...
# Cependant, nous allons réaliser une étude plus poussée pour déterminer le meilleur modèle. Nous
# allons notamment déterminer la significativité des coefficient des AR et des MA, et analyser
# l'autoccorélation des résidus de chaque modèle.
...
# Le BIC pénalise les modèles avec de grands ordre de retards
